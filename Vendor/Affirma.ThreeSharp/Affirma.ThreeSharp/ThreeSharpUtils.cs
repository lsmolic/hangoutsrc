...
{
    /// <summary>
    /// Determines the URL format to the S3 bucket and object
    /// </summary>
    public enum CallingFormat
    {
        ...
        /// <summary>
        /// http://mydomain.com/key -- a vanity domain which resolves to s3.amazonaws.com
        /// </summary>
        VANITY
    }

    /// <summary>
    /// This class contains utility functions for generating an S3 REST request
    /// </summary>
    public class ThreeSharpUtils
    {
        private class StringComparer : IComparer<string>
        {
            ...
            public StringComparer(CompareInfo compareInfo, CompareOptions compareOptions)
            {
            ...
            public int Compare(string a, string b)
            {
                ...
                return this.compareInfo.Compare(a, b, this.compareOptions);
            }
        }
        ...
        /// <summary>
        /// Generates an S3 canonical string
        /// </summary>
        public static string MakeCanonicalString(string bucket, string key, WebRequest request)
        {
            return MakeCanonicalString(bucket, key, new SortedList<string, string>(), request);
        }

        /// <summary>
        /// Generates an S3 canonical string
        /// </summary>
        public static string MakeCanonicalString(string bucket, string key, SortedList<string, string> query, WebRequest request)
        {
        ...
        /// Generates an S3 canonical string
        /// </summary>
        public static string MakeCanonicalString(string verb, string bucketName, string key, SortedList<string, string> queryParams, SortedList<string, string> headers, string expires)
        {
            StringBuilder buf = new StringBuilder();
            buf.Append(verb);
            buf.Append("\n");

            StringComparer comparer = new StringComparer(CompareInfo.GetCompareInfo(""), CompareOptions.StringSort);
            SortedList<string, string> interestingHeaders = new SortedList<string, string>(comparer);
            ...
            foreach (string header in interestingHeaders.Keys)
            {
                ...
                {
                    buf.Append(header).Append(":").Append((interestingHeaders[header]).Trim());
                }
                else
                {
            ...
            if (queryParams != null)
            {
                ...
                else if (queryParams.ContainsKey("torrent"))
                {
                    buf.Append("?torrent");
                }
                else if (queryParams.ContainsKey("logging"))
                {
                    buf.Append("?logging");
                }
                else if (queryParams.ContainsKey("requestPayment"))
                {
        ...
        /// <summary>
        /// Gets the current time in milliseconds since January 1st, 1970
        /// </summary>
        ...
        /// <summary>
        /// Converts a sorted list of query parameters to a querystring
        /// </summary>
        public static string ConvertQueryListToQueryString(SortedList<string, string> queryList)
        {
