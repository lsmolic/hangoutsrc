...
namespace Affirma.ThreeSharp.Wrapper
{
    ...
    public class ThreeSharpWrapper
    {
        ...
        /// <summary>
        /// Streams a MemoryStream to a bucket as an object
        /// </summary>
		public void AddPublicFileObject(String bucketName, String keyName, Stream fileStream)
        {
            using (ObjectAddRequest objectAddRequest = new ObjectAddRequest(bucketName, keyName))
            {
				objectAddRequest.LoadStreamWithStream(fileStream);
        ...
        /// </summary>
        public void CopyObject(String sourceBucketName, String sourceKey, String timestamp, String eTag, String destinationBucketName, String destinationKey)
        {
			using (ObjectCopyRequest objectCopyRequest = new ObjectCopyRequest(sourceBucketName, sourceKey, timestamp, eTag, destinationBucketName, destinationKey))
        ...
        {
            using (PaymentChangeRequest paymentChangeRequest = new PaymentChangeRequest(bucketName, payer))
            using (PaymentChangeResponse paymentChangeResponse = this.service.PaymentChange(paymentChangeRequest))
            {
                return paymentChangeResponse.StreamResponseToString();
            }
        }
