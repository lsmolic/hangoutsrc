...
namespace PureMVC.Patterns
{
    ...
    public class Facade : IFacade
	{
		...
		public T RetrieveProxy<T>() where T : IProxy
		{
			// The model is initialized in the constructor of the singleton, so this call should be thread safe.
			// This method is thread safe on the model.
			T proxy = (T)m_model.RetrieveProxy(typeof(T).Name);
            
            // Hangout Modification
            // Throw exception if Mediator is not retrieved
            if (proxy == null)
            {
                throw new Exception("No proxy registered with name: " + typeof(T).Name);
            }
            // End Hangout Modification
		...
		public T RetrieveMediator<T>() where T : IMediator
		{
			// The view is initialized in the constructor of the singleton, so this call should be thread safe.
			// This method is thread safe on the view.
			T mediator = (T)m_view.RetrieveMediator(typeof(T).Name);
            
            // Hangout Modification
            // Throw exception if Mediator is not retrieved
            if (mediator == null)
            {
                throw new Exception("No mediator registered with name: " + typeof(T).Name);
            }
		...
		// Hangout Modification - Added a generic RemoveMediator
		public T RemoveMediator<T>() where T : IMediator
		{
			return (T)RemoveMediator(typeof(T).Name);
		}
		// End Hangout Modification
		...
		// Hangout Modification - Added a generic HasMediator
		public bool HasMediator<T>() where T : IMediator
		{
			return HasMediator(typeof(T).Name);
		}
		// End Hangout Modification
