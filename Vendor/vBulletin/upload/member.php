...
foreach ($profileblock->locations AS $profilecategoryid => $location)
{
	if ($location)
	{
		if (strpos($location, 'profile_left') !== false)
		{
			$wrap = false;
		}
		else
...
foreach ($blocklist AS $blockid => $blockinfo)
{
	...
	{
		if (!empty($block_html) && strpos($blockinfo['hook_location'], 'profile_left') !== false)
		{
			...
			$template_hook["profile_left"] .= $block_html;
		}
		else
		{
