...
verify_seo_url('thread|js', $threadinfo);
...
if ($forum['allowratings'] == 1)
{
	...
	devdebug("threadinfo[rating] = $threadinfo[rating]");
...
if ($show['quickreply'])
{
	...
	// IMPORTANT, for now, we are disabling the require click feature for the quick reply
	// but the admin option still exists, which could be confusing to users
	// TODO, remove the admin option for disabling the quick reply by default
	$show['qr_require_click'] = 0;
