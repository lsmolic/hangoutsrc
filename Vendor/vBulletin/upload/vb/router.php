 ...
 * @version $Revision: 33203 $
 * @since $Date: 2009-11-05 13:53:49 -0600 (Thu, 05 Nov 2009) $
...
abstract class vB_Router
{
	 ...
	 */
	protected static function getEntryPath()
	{
		if (self::$entry_path)
		{
		...
		switch (FRIENDLY_URL)
		{
			case (self::METHOD_PATHINFO):
				$matches = array();
				if (preg_match('#^' . preg_quote(SCRIPT, '#') . '/([^\?]*)#', SCRIPTPATH, $matches) AND !empty($matches[1]))
				{
					self::$entry_path = $matches[1];
					break;
				}
			...
			default:
				self::$entry_path = vB::$vbulletin->input->clean_gpc('r', vB::$vbulletin->options['route_requestvar'], TYPE_STR);
				break;
		}

		if (!defined('VB_ROUTER_SEGMENT'))
		{
			throw (new vB_Exception_Router('No router segment defined in entry script'));
		}

		return trim(VB_ROUTER_SEGMENT . '/' . self::$entry_path . '/');
	}
	 ...
	 * @return vB_Route
	 */
	protected static function newRoute($route_path = false)
	{
		if (!$route_path)
		{
	 ...
	 * return string
	 */
	public static function getBaseRoutingURL($class_segment, $for_path = true)
	{
		if (!self::$base_url)
		{
		...
		switch (vB::$vbulletin->options['friendlyurl'])
		{
			case (self::METHOD_REQUEST):
	 ...
	 */
	public static function getURL($query_string = false)
	{
		if (!isset(self::$current_route))
		{
