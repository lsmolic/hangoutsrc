...
abstract class vB_Route
{
	 ...
	 * @return string							- The URL representing the route
	 */
	public function getCurrentURL(array $segments = null, array $parameters = null, $query_string = '')
	{
		...
		{
			$route_path = $this->_route_path ? substr($this->_route_path, strpos($this->_route_path, '/')) : '';
		}

		// Return the appropriate route path
		if ($this->_route_path == $this->_class_segment)
		{
			$url = trim(vB_Router::getBaseRoutingURL($this->_class_segment, false), '/?');
		}
		else
		{
			// Return the URL
			$url = trim(vB_Router::getBaseRoutingURL($this->_class_segment, true) . trim($route_path, '/'), '/?');
		}

		/* Disabling for now incase urls are cached
	 ...
	 * @return string
	 */
	public function getRoutePath($chop_class_segment = true, $inflate = false)
	{
		if ($inflate)
		{
	...
	*/
	...
	protected function buildRoutePath()
	{
		...
		if ($this->_modified OR !$this->_route_path)
		{
			...
			$this->_route_path = $this->_class_segment;
			$deferred_path = '';
			...
			foreach ($this->_segment_scheme AS $name => $segment)
			{
				...
				{
					$deferred_path .= ($deferred_path ? '/' : '') . $this->cleanSegment($this->_segments[$name]);
				}
				else if ($this->_segments[$name] != $segment['default'])
				{
					$this->_route_path = ($this->_route_path ? $this->_route_path . '/' : '') . ($deferred_path ? $deferred_path . '/' : '') . $this->cleanSegment($this->_segments[$name]);
					$deferred_path = '';
				}
			}
			...
			if (sizeof($this->_parameters))
			{
				...
				if ($deferred_path)
				{
					$this->_route_path = ($this->_route_path ? $this->_route_path . '/' : '') . $deferred_path;
				}

				$parameters = array();
				foreach ($this->_parameters AS $parameter)
				{
					$parameters[] = $this->cleanSegment($parameter);
				}

				$this->_route_path .= '/' . implode('/', $parameters);
			}
		}
	...
	/**
	 * Cleans a segment for output.
	 * 
	 * @param string $value
	 * @return string
	 */
	protected function cleanSegment($value)
	{
	...
	public function assertFriendlyUrl()
	{
		...
		if (!$fragment = vB_Friendly_Url::$decoded_fragment)
		{
			return;
		}

		if (FRIENDLY_URL === intval(vB::$vbulletin->options['friendlyurl']) AND $fragment == $this->getRoutePath(true, true))
		{
			return;
		}
		...
		if ($query = vB_Friendly_Url::get_cleaned_querystring())
		{
			$url .= (strpos($url, '?') ? '&' : '?') . urlimplode($query, false, true);
		}
