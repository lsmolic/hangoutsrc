...
abstract class vB_DM
{
	...
	*/
	...
	protected function setExistingField($fieldname, $value, $skip_undefined = false)
	{
		...
		// Validate the field
		if (!($error = $this->verify($fieldname, $value)) === true)
		{
	...
	*/
	protected function getField($fieldname, $ignore_errors = false)
	{
	...
	*/
	...
	protected function verifyPageText($pagetext, &$error, $noshouting = true)
	{
		...
		$pagetext = $this->preparseBBCodeVideo($pagetext);
	...
	/**
	 * Preparses bbcode vide.
	 *
	 * @param string $pagetext					- The text to parse
	 * @return string
	 */
	protected function preparseBBCodeVideo($pagetext)
	{
		...
		if (stripos($pagetext, '[video]') !== false)
		{
			...
			if (!empty($providers) AND preg_match_all('#\[video\](.*?)\[/video\]#si', $pagetext, $matches))
			{
				foreach ($matches[1] AS $key => $url)
				{
					...
					{
						if (preg_match('#' . $provider['regex_url'] . '#si', $url, $match))
						{
							break;
						}
					}
	...
	*/
	protected function getUpdateSQL($table)
	{
		...
		foreach ($this->table_fields[$table] AS $fieldname)
		{
			if (isset($this->set_fields[$fieldname]))
			{
				$sql .= "\r\n\t$fieldname = " . (isset($this->raw_fields[$fieldname]) ?
