...
abstract class vB_Item extends vB_Model
{
	...
	protected function loadCache()
	{
		...
		$context = $this->getCacheContext($this->required_info);

		// Fetch the cache info
		if ($info = vB_Cache::instance()->read($context, true))
		{
	...
	protected function writeCache()
	{
		...
		// Check if we've only loaded INFO_BASIC and whether we should use the cache
		if (($this->loaded_info == self::INFO_BASIC) AND !$this->cache_basic)
		{
			return;
		}

		// Create a context to identify the cache entry
		$context = $this->getCacheContext($this->loaded_info);

		// Add extra info that is not in item_properties
		$info = $this->saveCacheInfo();

		// Write the cache
		return vB_Cache::instance()->write($context, $info, 0, $this->getCacheEvents());
	}
