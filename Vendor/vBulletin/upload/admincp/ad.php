...
define('CVS_REVISION', '$RCSfile$ - $Revision: 33609 $');
...
if ($_POST['do'] == 'update')
{
	...
	*/
	...
	$template = build_ad_template($vbulletin->GPC['ad_location']);
	$template = "<div id='ad_".$ad['adlocation']."'>".$template."</div>";
	...
	// TODO: The original insert of the template. 
	$db->query_write("
		REPLACE INTO " . TABLE_PREFIX . "template SET
			styleid = 0,
			title = 'ad_" . $db->escape_string($vbulletin->GPC['ad_location']) . "',
...
if ($_POST['do'] == 'quickupdate')
{
	...
	if ($changes)
	{
		...
		foreach($changed_locations AS $location)
		{
			...
			$template = build_ad_template($location);
			$template_un = $template;
			$template = compile_template($template);
...
<?php
	...
	print_table_start();
	print_table_header($vbphrase['ad_selector'] . 'Ad Selector');
