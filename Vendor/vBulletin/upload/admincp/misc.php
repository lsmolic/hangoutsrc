...
define('CVS_REVISION', '$RCSfile$ - $Revision: 33631 $');
define('NOZIP', 1);
...
if ($_REQUEST['do'] == 'doindextypes')
{
	...
	echo '<p>' . $vbphrase['building_search_index'] . ' ' .
				 $indexed_types[$vbulletin->GPC['indextypes']]['class']  . ' ' .
...
if ($_REQUEST['do'] == 'rebuildthumbs')
{
	...
	while ($attachment = $db->fetch_array($attachments))
	{
		...
		echo construct_phrase($vbphrase['processing_x'], "$vbphrase[attachment] : ($attachment[extension])");
...
if ($_REQUEST['do'] == 'buildpostcache')
{
	...
	while ($post = $db->fetch_array($posts))
	{
		...
		if ($vbulletin->forumcache["$post[forumid]"]['options'] & $vbulletin->bf_misc_forumoptions['styleoverride'])
		{
			...
			$vbulletin->stylevars = unserialize($stylelist["$styleid"]['newstylevars']);
