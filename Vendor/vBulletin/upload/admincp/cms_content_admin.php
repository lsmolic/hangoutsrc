...
switch($vbulletin->GPC['do'])
{
	...
	case 'clear_cache':
		...
		if (method_exists(vB_Cache::instance(), 'clean'))
		{
			...
			echo $vbphrase['cache_cleared'];
		}
		else
		{
			echo $vbphrase['no_cache_clean'];
		}
		print_cp_footer();
		break;
	...
	case 'perpage': //Here are are updating the number of rows per page
		...
		{
			setTimeout('document.location=\'./cms_content_admin.php?do=$redirect_do\'', 3000);
		}
		</script>";
		print_cp_header('' , 'show_redirect()');
		echo $vbphrase['saved_perpage'];
		print_cp_footer();
		break;
	...
	case 'fix_nodes':
		...
		vBCms_ContentManager::fixNodeLR();
		echo $vbphrase['update_complete'];
		print_cp_footer();
		break;
