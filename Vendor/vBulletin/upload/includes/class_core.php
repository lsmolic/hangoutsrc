...
define('FILE_VERSION', '4.0.0 Beta 4'); // this should match install.php
define('SIMPLE_VERSION', '400b4'); // see vB_Datastore::check_options()
...
*
* @package	vBulletin
* @version	$Revision: 33595 $
* @date		$Date: 2009-11-17 10:16:36 -0600 (Tue, 17 Nov 2009) $
*/
class vB_Database
{
	/**
	...
	*/
	function fetch_array($queryresult, $type = DBARRAY_ASSOC)
	{
		...
		{
			($hook = vBulletinHook::fetch_hook('database_fetch_array')) ? eval($hook) : false;
		}
	...
	*/
	function fetch_row($queryresult)
	{
		if (class_exists('vBulletinHook'))
		{
			($hook = vBulletinHook::fetch_hook('database_fetch_row')) ? eval($hook) : false;
		}
...
class vB_Input_Cleaner
{
	...
	*/
	function vB_Input_Cleaner(&$registry)
	{
		...
		// translate and clean the friendly url so WOL can understand it
		require_once(CWD . '/includes/class_friendly_url.php');
		$translatedpath = $this->xss_clean($this->strip_sessionhash(vB_Friendly_Url::decode_friendly_url()));
		...
		$registry->wolpath = $translatedpath ? $translatedpath : $this->fetch_wolpath();
		...
		define('VB_HTTP_HOST', trim($http_host));
	}
	...
	*/
	...
	function fetch_basepath($rel_modifier = false)
	{
		...
		{
			if ((!$_SERVER['SERVER_NAME'] AND !$_SERVER['HTTP_HOST']) OR (!$_SERVER['SCRIPT_NAME'] AND !$_SERVER['PHP_SELF']) AND
				((!$_ENV['SERVER_NAME'] AND !$_ENV['HTTP_HOST']) OR (!$_ENV['SCRIPT_NAME'] AND !$_ENV['PHP_SELF'])))
			{
				$basepath = vB::$vbulletin->options['bburl'];
			}
			else
			{
	...
	{
		if ($this->registry->relpath != '')
		{
			return $this->registry->relpath;
		}

		$baseurl = $this->fetch_basepath();
		$currenturl = parse_url($baseurl);
		$currenturl = $currenturl['scheme'] . '://' . $currenturl['host'] . $this->fetch_scriptpath();

		// Note: Return $currenturl if all else fails
		// return $currenturl;

		// get current path relative to basepath
		$relpath = explode('/', ltrim(parse_url($currenturl, PHP_URL_PATH), '/'));
		$query = parse_url($currenturl, PHP_URL_QUERY);
		$query = $query ? "?$query" : '';
		$script = urldecode(array_pop($relpath));

		$basepath = explode('/', trim(parse_url($baseurl, PHP_URL_PATH), '/'));
		...
		{
			$relurl = str_repeat('../', $diff);
		}

		$relpath = implode('/', $relpath);
		// TODO: may need a leading / if $relpath
		$relpath = $relpath ? $relpath . '/' : '';
		$relurl .= $relpath . $script . $query;

		return $this->registry->relpath = $relurl;
	}
...
class vB_Template
{
	...
	*/
	...
	{
		($hook = vBulletinHook::fetch_hook('template_register_var')) ? eval($hook) : false;
	...
	*/
	public function render($suppress_html_comments = false)
	{
		...
		// TODO: Fix css as files with RTL
		//if ($vbulletin->options['storecssasfile'])
		//{
		//	$vbcsspath = 'clientscript/vbulletin_css/style' . str_pad($vbulletin->userinfo['styleid'], 5, '0', STR_PAD_LEFT) . '/';
		//}
		//else
		//{
...
class vB_Template_Runtime
{
	...
	public static function parsePhrase($phraseName)
	{
		...
		return construct_phrase($vbphrase[$phraseName], $arg_list);
	}
			...
			switch($base_stylevar['datatype'])
			{
				...
				case 'background':
					$output = $base_stylevar['color'] . ' ' . (!empty($base_stylevar['image']) ? "$base_stylevar[image]" : 'none') . ' ' .
						$base_stylevar['repeat'] . ' ' . $base_stylevar['x'] . $base_stylevar['units'] . ' ' . $base_stylevar['y'] .
						$base_stylevar['units'];
				break;
				...
				case 'margin':
					if (isset($base_stylevar['same']) AND $base_stylevar['same'])
					{
						$output = $base_stylevar['top'] . $base_stylevar['units'];
					}
					else
					{
						$output =
							$base_stylevar['top']  . $base_stylevar['units'] . ' ' .
							$base_stylevar['right'] . $base_stylevar['units'] . ' ' .
							$base_stylevar['bottom'] . $base_stylevar['units'] . ' ' .
							$base_stylevar['left'] . $base_stylevar['units'];
					}
				break;
			}
		}
	...
	public static function runMaths($str)
	{
		...
		{
			//hack: if the match string is invalid we can get a php parse error here.
