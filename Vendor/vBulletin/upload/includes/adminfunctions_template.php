...
function delete_style_css_directory($styleid)
{
	$styledir = 'clientscript/vbulletin_css/style' . str_pad($styleid, 5, '0', STR_PAD_LEFT);
	foreach (new DirectoryIterator($styledir) as $file)
	{
		if (!is_dir($file))
		{
...
function write_style_css_directory($styleid, $parentlist)
{
	global $vbulletin;
	...
	{
		delete_style_css_directory($styleid);
	}
...
function build_style($styleid, $title = '', $actions = array(), $parentlist = '', $indent = '')
{
	...
	if ($styleid != -1)
	{
		...
		if ($vbulletin->options['storecssasfile'])
		{
			if (!write_style_css_directory($styleid, $parentlist))
			{
...
function compile_template($template)
{
	...
	else
	{
		...
		if (!empty($errors))
		{
			...
			print_form_header('', '', 0, 1, '', '65%');
			print_table_header($vbphrase['vbulletin_message']);
			print_description_row("<ul><li>" . implode("</li>\n<li>", $errors) . "</li></ul>");
			print_table_footer(2, construct_button_code($vbphrase['go_back'], 'javascript:history.back(1)'));
			print_cp_footer();
			exit;
		}
...
function xml_import_style($xml = false, $styleid = -1, $parentid = -1, $title = '', $anyversion = false, $displayorder = 1, $userselect = true)
{
	...
	//$vbulletin->db->query_write("BEGIN");
	...
	if ($master)
	{
		build_adminutil_text('master_style_import', serialize(
			array('last_import' => TIMENOW)
		));
	}
