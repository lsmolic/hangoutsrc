...
function strip_blank_ascii($text, $replace)
{
	...
	if ($blanks === null AND trim($vbulletin->options['blankasciistrip']) != '')
	{
		...
		$charset_unicode = (strtolower(vB_Template_Runtime::fetchStyleVar('charset')) == 'utf-8');
...
function strip_bbcode($message, $stripquotes = false, $fast_and_dirty = false, $showlinks = true)
{
	...
	else
	{
		...
		// video tags need stripping too
		$find[] = '#\[video.*\].+\[\/video\]#siU';
		$replace[] = '';
...
{
	$perpage = fetch_perpage($perpage, $maxperpage = 20, $defaultperpage = 20);
...
function construct_page_nav($pagenumber, $perpage, $results, $address, $address2 = '', $anchor = '', $seolink = '', $objectinfo = array(), $pageinfo = array())
{
	...
	$show_prior_elipsis = $show_after_elipsis = ($totalpages > 10) ? 1 : 0;
...
function create_full_url($url)
{
	...
	{
		// if we don't have VB_HTTP_HOST, then this isn't going to work correctly,
		...
		if (VB_HTTP_HOST)
		{
			...
			if ($vbulletin->options['friendlyurl'] == 3)
			{
				$friendlies = vB_Friendly_Url::fetch_friendlies();
				$vars = array();
				foreach ($friendlies AS $friendinfo)
				{
					$vars[] = $friendinfo['rewritevar'];
				}
				if (preg_match('#/(' . implode('|', $vars) . ')/$#', $dirpath))
				{
					$url = preg_replace('#^(' . implode('|', $vars) . ')/#', '', $url);
				}
			}

			$dirpath .= $url;

			$url = $method . VB_HTTP_HOST . $dirpath;
		}
	}
...
function cache_ordered_forums($getcounters = 0, $getinvisibles = 0, $userid = 0)
{
	...
	if ($getcounters)
	{
		...
		{
			$tachyjoin = '';
			$counter_select = 'forum.forumid, lastpost, lastposter, lastposterid, lastthread, lastthreadid, lasticonid, threadcount, replycount, lastpostid, lastprefixid';
		}
...
*/
function construct_quick_nav($navpopup = array(), $parentid = -1, $addbox = true)
{
...
function verify_seo_url($link, $linkinfo, $pageinfo = null, $primaryid = null, $primarytitle = null)
{
	...
	// if we are not passed any linkinfo, there is nothing to check the url against
	// so simply return without doing anything (could be an invalid link)
	if (empty($linkinfo))
	{
		return;
	}

	// if not given a pageinfo array, clean the query string and use that
	if (empty($pageinfo))
	{
		$pageinfo = vB_Friendly_Url::get_cleaned_querystring();
	}

	$friendlyurl = vB_Friendly_Url::fetchLibrary($vbulletin, $link, $linkinfo, $pageinfo, $primaryid, $primarytitle);
	// verify and perform a redirect if we are not at the canonical url for this page
	$friendlyurl->verify_canonical_url(true);
}
