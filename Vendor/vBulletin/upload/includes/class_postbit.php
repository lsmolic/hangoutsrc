...
*
* @package 		vBulletin
* @version		$Revision: 33609 $
* @date 		$Date: 2009-11-17 13:12:52 -0600 (Tue, 17 Nov 2009) $
*
*/
	...
	*/
	function construct_postbit(&$post)
	{
		...
		if ($post['isfirstshown'])
		{
			...
			$ad_location['thread_first_post_content'] = vB_Template::create('ad_thread_first_post_content')->render();
		}
		else if ($post['islastshown'])
		{
			$ad_location['thread_last_post_content'] = vB_Template::create('ad_thread_last_post_content')->render();
		}
		...
		$templater = vB_Template::create($this->template_prefix . $this->templatename);
			if ($vbulletin->products['vbcms'])
			{
				...
				if (vB::$vbulletin->userinfo['permissions']['cms']['cancreate'][0] != -1)
				{
					$templater->register('promote_sectionid', vB::$vbulletin->userinfo['permissions']['cms']['cancreate'][0]);
					$templater->register('articletypeid', vB_Types::instance()->getContentTypeID('vBCms_Article'));
					$promote_url = vB_Route::create('vBCms_Route_Content', '1/addcontent/',
						'contenttypeid=' . vB_Types::instance()->getContentTypeID('vBCms_Article') .
						"&amp;postid=" . $post['postid'] . '&amp;parentid=1')->getCurrentURL();
