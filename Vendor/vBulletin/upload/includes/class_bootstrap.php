...
class vB_Bootstrap
{
	...
	*/
	public function process_templates()
	{
		...
		global $pagenumber, $headinclude, $header, $footer;
...
class vB_Bootstrap_Forum extends vB_Bootstrap
{
	...
	*/
	public function load_show_variables()
	{
		...
		$show['forumdesc'] = (
			$vbulletin->options['nav_forumdesc']
			AND !empty($foruminfo['description'])
			AND trim($foruminfo['description']) != ''
			AND in_array(THIS_SCRIPT, array('newthread', 'newreply', 'forumdisplay', 'showthread', 'announcement', 'editpost', 'poll', 'report', 'sendmessage', 'threadrate'))
		);
