...
class vB_DataManager
{
	...
	*/
	function verify_pagetext(&$pagetext, $noshouting = true)
	{
		...
		$pagetext = $this-> preparse_bbcode_video($pagetext);

		return true;
	}

	function preparse_bbcode_video($pagetext)
	{
		...
		if (stripos($pagetext, '[video]') !== false)
		{
			$bbcodes = $this->registry->db->query_read_slave("
				...
				ORDER BY priority
			");
			while ($bbcode = $this->registry->db->fetch_array($bbcodes))
			{
			...
			if (!empty($providers) AND preg_match_all('#\[video\](.*?)\[/video\]#si', $pagetext, $matches))
			{
				foreach ($matches[1] AS $key => $url)
				{
					...
					if ($match)
					{
						...
						else if ($provider['regex_scrape'] AND $this->registry->options['bbcode_video_scrape'] > 0 AND $scraped < $this->registry->options['bbcode_video_scrape'])
						{
