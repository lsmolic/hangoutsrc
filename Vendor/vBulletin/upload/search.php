	 ...
	 $searchterms=array())
{
	...
	else
	{
		...
		$template->register('type_options',
			vB_Search_Searchtools::renderTypeOptions(vB_Search_Searchtools::get_type_options(), $prefs['type']));
	}
...
function set_criteria_from_vbform($user, $criteria)
{
	...
	if (!$vbulletin->GPC_exists['sortby'])
	{
		$vbulletin->GPC['sortby'] = 'dateline';
	}
	...
	else
	{
		if ($vbulletin->GPC['include'])
		{
			...
			if (is_array($list))
			{
				...
				$criteria->add_forum_filter($list, false);
			}
		}
		...
		if ($vbulletin->GPC['exclude'])
		{
			...
			{
				$list = array_map('intval', $list);
				$criteria->add_excludeforum_filter($list);
			}
		}
	}
}
